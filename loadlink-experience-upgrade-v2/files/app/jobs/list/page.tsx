...
function getOwnerKey() {
  ...
  const key = `${Date.now()}-${Math.random().toString(36).slice(2)}-${typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : Math.random().toString(36).slice(2)}`;
  ...
  async function submitJob(event: FormEvent) {
    ...
    try {
      ...
      try {
        ...
        // The owner key keeps no-login posting and chat available.
      }
    ...
    <header className={`sticky top-0 z-50 border-b ${darkMode ? "border-white/10 bg-black" : "border-black/10 bg-white"}`}>
      <div className="grid h-20 grid-cols-[92px_1fr_52px] items-center px-4">
        <div className="flex items-center gap-2">
          ...
          <Link href="/login" className={`flex h-10 w-10 items-center justify-center rounded-full border ${darkMode ? "border-yellow-400/60 bg-yellow-400 text-black" : "border-black/10 bg-white text-black shadow-sm"}`} aria-label="Log in or sign up"><UserPlusIcon /></Link>
        </div>
