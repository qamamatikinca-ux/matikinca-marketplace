...
function getOwnerKeys() {
  ...
  try {
    ...
    // Old malformed device data is ignored without breaking chat.
  }
...
function activityText(conversation: Conversation) {
  ...
  const difference = Date.now() - new Date(conversation.other_last_seen).getTime();
  if (difference < 90_000) return "Active now";
  if (difference < 3_600_000) return `Active ${Math.max(1, Math.round(difference / 60_000))} min ago`;
  if (difference < 86_400_000) return `Active ${Math.max(1, Math.round(difference / 3_600_000))} hr ago`;
...
function replyText(minutes: number | null) {
  if (!minutes || minutes < 1) return "Reply time will appear after a few responses";
  if (minutes <= 5) return "Usually replies within a few minutes";
  if (minutes < 60) return `Usually replies within about ${minutes} min`;
  if (minutes < 1440) return `Usually replies within about ${Math.max(1, Math.round(minutes / 60))} hr`;
...
function cleanError(error: unknown, fallback: string) {
  ...
  if (/fetch|network/i.test(message)) return "Connection interrupted. Check your signal and try again.";
...
export default function MessagesPage() {
  ...
  const loadConversations = useCallback(async (preferredId?: string) => {
    ...
    const buyerResult = await supabase.rpc("get_buyer_guest_threads", { p_buyer_key: buyerKey });
    if (buyerResult.error) throw buyerResult.error;

    const buyerRows = ((buyerResult.data || []) as ConversationRow[]).map((row) => ({
      ...row,
      accessKey: buyerKey,
      role: "buyer" as const,
    ...
    for (const ownerKey of ownerKeys) {
      const ownerResult = await supabase.rpc("get_owner_guest_threads", { p_owner_key: ownerKey });
    ...
    const nextId = preferredId || selectedIdRef.current || (window.innerWidth >= 768 ? rows[0]?.id || "" : "");
  ...
  const loadMessages = useCallback(async (conversation: Conversation, showLoader = false) => {
    ...
    async function initialise() {
      try {
        ...
        if (listingId) {
          ...
          window.history.replaceState({}, "", openedId ? `/messages?thread=${openedId}` : "/messages");
        } else {
        ...
        if (active) setError(cleanError(initialiseError, "Chat could not open."));
      } finally {
  ...
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: messagesLoading ? "auto" : "smooth" });
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-black px-6 text-white">
        <div className="text-center">
          ...
          <p className="mt-6 text-xs font-black uppercase tracking-[.22em] text-[#f6b800]">LoadLink messages</p>
  ...
  return (
    <main className="h-[100dvh] overflow-hidden bg-[#eeeae0] text-black">
      <header className="grid h-[72px] grid-cols-[56px_1fr_56px] items-center border-b border-black/10 bg-black px-3 text-white md:h-20 md:grid-cols-[120px_1fr_120px] md:px-5">
        <Link href="/" className="inline-flex items-center gap-2 text-sm font-black" aria-label="Back to LoadLink home">
      ...
      <div className="mx-auto grid h-[calc(100dvh-72px)] max-w-[1500px] md:h-[calc(100dvh-80px)] md:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[380px_minmax(0,1fr)_300px]">
        <aside className={`${selectedId ? "hidden md:flex" : "flex"} min-h-0 flex-col border-r border-black/10 bg-white`}>
          <div className="border-b border-black/10 p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[.22em] text-[#b88900]">LoadLink</p>
                <h1 className="mt-1 text-3xl font-black tracking-[-.04em]">Messages</h1>
              </div>
              <span className="rounded-full bg-black px-3 py-1.5 text-xs font-black text-[#f6b800]">
                {conversations.reduce((total, conversation) => total + conversation.unreadCount, 0)} unread
              </span>
            </div>
            ...
            {visibleConversations.length ? (
              visibleConversations.map((conversation) => {
                ...
                return (
                  ...
                  >
                    <span className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-black text-sm font-black text-[#f6b800]">
                      {conversation.other_name.trim().slice(0, 2).toUpperCase() || "LL"}
                      {conversation.other_last_seen && Date.now() - new Date(conversation.other_last_seen).getTime() < 90_000 ? (
                        <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-[#25b85a]" />
                      ) : null}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="flex items-start justify-between gap-2">
                        <strong className="truncate text-sm font-black">{conversation.other_name}</strong>
                        <span className="shrink-0 text-[10px] font-bold text-black/40">{formatConversationDate(conversation.last_message_at)}</span>
                      </span>
                      <span className="mt-1 block truncate text-xs font-bold text-[#8a6700]">{conversation.listing_title}</span>
                      <span className="mt-1 flex items-center justify-between gap-3">
                        <span className={`truncate text-xs ${conversation.unreadCount ? "font-black text-black" : "font-medium text-black/50"}`}>
                          {conversation.last_message_has_attachment ? "Attachment · " : ""}
              ...
              <div className="p-8 text-center">
                ...
                <h2 className="mt-5 text-xl font-black">No conversations yet</h2>
                <p className="mt-2 text-sm leading-6 text-black/55">Open a listing and tap Message to start a private conversation.</p>
                <Link href="/jobs" className="mt-5 inline-flex rounded-xl bg-[#f6b800] px-5 py-3 text-xs font-black uppercase tracking-wide text-black">
                  Browse listings
                </Link>
              </div>
            )}
        ...
        <section className={`${selectedId ? "flex" : "hidden md:flex"} min-h-0 flex-col bg-[#f3f0e8]`}>
          {selectedConversation ? (
            <>
              <header className="flex min-h-[78px] items-center gap-3 border-b border-black/10 bg-white px-3 py-3 md:px-5">
                <button type="button" onClick={returnToInbox} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-black/10 text-xl font-black md:hidden" aria-label="Back to conversations">
                  ←
                </button>
                <span className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-black text-xs font-black text-[#f6b800]">
                  {selectedConversation.other_name.trim().slice(0, 2).toUpperCase() || "LL"}
                  {selectedConversation.other_last_seen && Date.now() - new Date(selectedConversation.other_last_seen).getTime() < 90_000 ? (
                    <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-[#25b85a]" />
                  ) : null}
                </span>
                <div className="min-w-0 flex-1">
                  <h2 className="truncate text-base font-black md:text-lg">{selectedConversation.other_name}</h2>
                  <p className={`truncate text-xs font-bold ${selectedConversation.other_typing ? "text-[#168b42]" : "text-black/45"}`}>
                    {activityText(selectedConversation)}
                  </p>
                  <p className="hidden truncate text-[11px] font-semibold text-[#8a6700] sm:block">{replyText(selectedConversation.average_reply_minutes)}</p>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  {selectedConversation.other_phone ? (
                    ...
                    >
                      <PhoneIcon />
                      <span className="ml-2 hidden text-xs font-black uppercase md:inline">Call</span>
                    </a>
                  ) : null}
                  <button
                    type="button"
              ...
              <div className="border-b border-[#d7b33b]/35 bg-[#fff7dc] px-4 py-2.5 text-[11px] font-semibold leading-5 text-black/60">
                Messages and attachments are protected in transit and stored privately. Only people in this conversation can access them.
              </div>
                ...
                {messagesLoading ? (
                          ...
                          {showDay ? (
                            <div className="my-5 flex justify-center">
                              <span className="rounded-full border border-black/5 bg-white/80 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-black/45">
                                {new Intl.DateTimeFormat("en-ZA", { weekday: "short", day: "2-digit", month: "short" }).format(new Date(message.created_at))}
                              </span>
                            </div>
                          ) : null}
                          <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[86%] rounded-2xl px-4 py-3 shadow-sm sm:max-w-[72%] ${
                              mine ? "rounded-br-sm bg-black text-white" : "rounded-bl-sm border border-black/5 bg-white text-black"
                            }`}>
                              {message.attachment_id ? (
                                ...
                                >
                                  <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${mine ? "bg-[#f6b800] text-black" : "bg-black text-[#f6b800]"}`}>
                                    <PaperclipIcon />
                                  </span>
                                  <span className="min-w-0 flex-1">
                                    <strong className="block truncate text-xs font-black">{message.file_name || "Attachment"}</strong>
                                    <span className={`mt-0.5 block text-[10px] font-semibold ${mine ? "text-white/55" : "text-black/45"}`}>
                                      {fileSizeLabel(message.file_size)} · Tap to open
                                    </span>
                                  </span>
                                </button>
                              ) : null}
                              {message.body && (!message.attachment_id || message.body !== "Shared an attachment") ? (
                  ...
                  <div className="flex h-full items-center justify-center px-5 text-center">
                    <div className="max-w-sm">
                      ...
                      <h3 className="mt-5 text-2xl font-black">Start the conversation</h3>
                      <p className="mt-2 text-sm leading-6 text-black/50">Ask about availability, location, timing, price or the requirements for this listing.</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <form onSubmit={send} className="border-t border-black/10 bg-white p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] sm:p-4">
                <input
                  ref={fileInputRef}
                  type="file"
                ...
                <p className="mx-auto mt-2 max-w-3xl text-center text-[10px] font-semibold text-black/35">Photos and documents up to 5 MB</p>
              </form>
            </>
          ) : (
            <div className="flex h-full items-center justify-center p-8 text-center">
              <div className="max-w-md">
                ...
                <p className="mt-6 text-xs font-black uppercase tracking-[.22em] text-[#b88900]">LoadLink chat</p>
                <h2 className="mt-2 text-4xl font-black tracking-[-.05em]">Your logistics conversations in one place</h2>
                <p className="mt-4 text-sm font-medium leading-7 text-black/50">Select a conversation to message a listing owner or respond to someone interested in your post.</p>
              </div>
            </div>
          )}
        </section>
        ...
        <aside className="hidden min-h-0 overflow-y-auto border-l border-black/10 bg-white p-5 xl:block">
          {selectedConversation ? <ConversationDetails conversation={selectedConversation} /> : null}
        </aside>
      </div>
    </main>
  );
...
function ConversationDetails({ conversation }: { conversation: Conversation }) {
  return (
    <div>
      <p className="text-[10px] font-black uppercase tracking-[.22em] text-[#b88900]">Conversation details</p>
      <div className="mt-5 flex items-center gap-3">
        <span className="flex h-12 w-12 items-center justify-center rounded-full bg-black text-sm font-black text-[#f6b800]">
          {conversation.other_name.trim().slice(0, 2).toUpperCase() || "LL"}
        </span>
        <div className="min-w-0">
          <h3 className="truncate font-black">{conversation.other_name}</h3>
          <p className="mt-0.5 text-xs font-semibold text-black/45">{activityText(conversation)}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-2">
        <Link href={`/jobs#job-${conversation.listing_id}`} className="flex items-center justify-center rounded-xl border border-black/10 px-4 py-3 text-xs font-black uppercase tracking-wide">
          View listing
        </Link>
        {conversation.other_phone ? (
          <a href={`tel:${conversation.other_phone.replace(/\s/g, "")}`} className="flex items-center justify-center rounded-xl bg-black px-4 py-3 text-xs font-black uppercase tracking-wide text-[#f6b800]">
            Call contact
          </a>
        ) : null}
      </div>
      ...
      <div className="mt-6 rounded-xl border border-[#e5c34c]/35 bg-[#fff8de] p-4">
        ...
        <p className="mt-2 text-xs font-medium leading-5 text-black/55">Confirm listing details before paying. Avoid sending passwords, PINs or one-time verification codes.</p>
      </div>
    </div>
  );
}
...
function PaperclipIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="m20.5 11.5-7.8 7.8a5 5 0 0 1-7.1-7.1l8.5-8.5a3.5 3.5 0 0 1 5 5l-8.5 8.5a2 2 0 0 1-2.8-2.8l7.8-7.8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function PhoneIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M7.2 3h3l1.5 4-2 1.5a15 15 0 0 0 5.8 5.8l1.5-2 4 1.5v3A3.2 3.2 0 0 1 17.8 20C10.2 20 4 13.8 4 6.2A3.2 3.2 0 0 1 7.2 3Z" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" />
    </svg>
  );
}
