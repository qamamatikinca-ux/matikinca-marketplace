...
import { ACTIVE_ACCOUNT_STORAGE_KEY, LEGACY_STATE_OWNER_STORAGE_KEY } from "@/lib/chatKeys";
...
function removeLegacyOwnershipKeys() {
  [
    "loadlink-chat-key",
    "loadlink-chat-keys",
    "loadlink-owned-job-keys",
    "loadlink-device-key",
...
export async function syncAccountState() {
  ...
  const { data: remote, error: readError } = await supabase
    ...
    .select("recent_viewed,recent_portals,liked_listings")
    .eq("user_id", user.id)
  ...
  if (!readError) {
    const { error: writeError } = await supabase.from("user_account_state").upsert(
      {
        user_id: user.id,
        // Keep the old columns empty during the transition. Ownership and messaging
        // are now authorised exclusively by the signed-in Supabase account.
        buyer_keys: [],
        owned_job_keys: {},
