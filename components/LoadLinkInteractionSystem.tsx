...
function enhanceDateInput(input: HTMLInputElement) {
  if (input.type !== "date" || input.dataset.loadlinkFutureDate !== "true") return;
  input.readOnly = true;
  input.dataset.loadlinkCalendarReady = "true";
  input.setAttribute("aria-haspopup", "dialog");
  input.setAttribute("aria-label", input.getAttribute("aria-label") || input.closest("label")?.textContent?.trim() || "Choose date");
}

function enhanceChatTools(pathname: string) {
  if (!pathname.startsWith("/messages")) return;
  document.querySelectorAll<HTMLButtonElement>('button[aria-label="Open message actions"], button[data-loadlink-message-tools-trigger="true"]').forEach((button) => {
    button.dataset.loadlinkMessageToolsTrigger = "true";
    button.classList.add("loadlink-message-tools-trigger");
    button.setAttribute("aria-label", button.getAttribute("aria-expanded") === "true" ? "Close message tools" : "Open message tools");
  });
}
...
export default function LoadLinkInteractionSystem() {
  ...
  useEffect(() => {
    const syncTheme = () => setDarkMode(document.documentElement.dataset.loadlinkTheme === "dark" || document.documentElement.classList.contains("dark"));
    syncTheme();
  ...
  useEffect(() => {
    ...
    const onDateClick = (event: MouseEvent) => {
      ...
      const input = element?.closest('input[type="date"][data-loadlink-calendar-ready="true"]');
      ...
      setCalendar({
        input,
        label: input.closest("label")?.querySelector("span")?.textContent?.trim() || input.getAttribute("aria-label") || "Choose date",
        min: input.min || "",
        max: input.max || "",
      });
    };
  ...
  function commitDate() {
    ...
    calendar.input.focus({ preventScroll: true });
    setCalendar(null);
  }
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .loadlink-message-tools-trigger[aria-expanded="true"]::after { content: "Close"; }
        ...
        input[data-loadlink-calendar-ready="true"] { cursor: pointer; }
        body:has([data-loadlink-calendar-sheet="true"]) { overflow: hidden; }
      `}</style>

      {calendar ? (
        <div
          data-loadlink-calendar-sheet="true"
          className="fixed inset-0 z-[2147483590] flex items-end justify-center bg-black/30 p-0 backdrop-blur-[16px] sm:items-center sm:p-4"
          role="dialog"
          aria-modal="true"
          aria-labelledby="loadlink-calendar-title"
          onMouseDown={(event) => { if (event.target === event.currentTarget) setCalendar(null); }}
        >
          <section className={`w-full max-w-[520px] overflow-hidden rounded-t-[34px] border shadow-[0_30px_100px_rgba(0,0,0,.36)] backdrop-blur-3xl backdrop-saturate-150 sm:rounded-[34px] ${darkMode ? "border-white/15 bg-black/[.68] text-white" : "border-white/75 bg-white/[.76] text-black"}`}>
            <div className={`border-b px-5 pb-4 pt-5 ${darkMode ? "border-white/10" : "border-black/8"}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className={`text-[10px] font-black uppercase tracking-[.13em] ${darkMode ? "text-[#f6b800]" : "text-[#a97900]"}`}>{calendar.label}</p>
                  <h2 id="loadlink-calendar-title" className="mt-1 text-[28px] font-black tracking-[-.045em]">{title}</h2>
                </div>
                <button type="button" onClick={() => setCalendar(null)} className={`flex h-10 w-10 items-center justify-center rounded-full border ${darkMode ? "border-white/15 bg-white/[.05]" : "border-black/10 bg-white/45"}`} aria-label="Close calendar"><LoadLinkIcon name="close" size={17} /></button>
              </div>
              <div className={`mt-4 inline-flex rounded-full border p-1 ${darkMode ? "border-white/12 bg-white/[.04]" : "border-black/8 bg-black/[.025]"}`}>
                <button type="button" onClick={() => { const now = new Date(); choose(now); setShownMonth(monthStart(now)); }} className="rounded-full bg-[#f6b800] px-4 py-2 text-[11px] font-black text-black">Today</button>
                <span className={`px-4 py-2 text-[11px] font-bold ${darkMode ? "text-white/60" : "text-black/55"}`}>{selected ? new Date(`${selected}T12:00:00`).toLocaleDateString("en-ZA", { day: "numeric", month: "short", year: "numeric" }) : "Choose a date"}</span>
              </div>
            </div>

            <div className="px-5 py-4">
              <div className="flex items-center justify-between gap-4">
                <button type="button" onClick={() => setShownMonth((value) => new Date(value.getFullYear(), value.getMonth() - 1, 1, 12))} className={`flex h-10 w-10 items-center justify-center rounded-full border ${darkMode ? "border-white/12 bg-white/[.04]" : "border-black/8 bg-white/35"}`} aria-label="Previous month"><LoadLinkIcon name="chevronLeft" size={18} /></button>
                <strong className="text-base font-black">{shownMonth.toLocaleDateString("en-ZA", { month: "long", year: "numeric" })}</strong>
                <button type="button" onClick={() => setShownMonth((value) => new Date(value.getFullYear(), value.getMonth() + 1, 1, 12))} className={`flex h-10 w-10 items-center justify-center rounded-full border ${darkMode ? "border-white/12 bg-white/[.04]" : "border-black/8 bg-white/35"}`} aria-label="Next month"><LoadLinkIcon name="chevronRight" size={18} /></button>
              </div>
                ...
                {days.map((date, index) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={disabled}
                      onClick={() => choose(date)}
                      className={`mx-auto flex aspect-square w-[42px] max-w-full items-center justify-center rounded-full text-sm font-black transition active:scale-95 disabled:opacity-20 ${active ? "bg-[#f6b800] text-black shadow-[0_8px_24px_rgba(246,184,0,.20)]" : today ? (darkMode ? "border border-white/25 bg-white/[.04]" : "border border-black/15 bg-white/25") : "hover:bg-current/[.05]"}`}
                    >
                      {date.getDate()}
                    </button>
                  );
            ...
            <div className={`flex items-center justify-between gap-3 border-t px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 ${darkMode ? "border-white/10" : "border-black/8"}`}>
              <button type="button" onClick={() => setCalendar(null)} className={`h-12 rounded-[16px] border px-5 text-sm font-black ${darkMode ? "border-white/14 bg-white/[.04]" : "border-black/10 bg-white/30"}`}>Cancel</button>
              <button type="button" disabled={!selected} onClick={commitDate} className="h-12 flex-1 rounded-[16px] bg-[#f6b800] px-5 text-sm font-black text-black disabled:opacity-40">Choose date</button>
            </div>
          </section>
        </div>
      ) : null}
