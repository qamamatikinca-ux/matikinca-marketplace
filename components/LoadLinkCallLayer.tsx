...
type SignalRow = { signal_type: "offer" | "answer" | "ice" | "hangup"; payload: Record<string, unknown>; sender_user_id: string };
...
export default function LoadLinkCallLayer() {
  ...
  const endLocal = useCallback((message = "Call ended") => {
    ...
    heartbeatRef.current = null; countdownRef.current = null;
    signalChannelRef.current?.unsubscribe(); signalChannelRef.current = null;
    ...
    setActive(null); setRemaining(0); setMuted(false); setStatus(message);
  }, []);

  const sendSignal = useCallback(async (sessionId: string, type: SignalRow["signal_type"], payload: Record<string, unknown>) => {
  ...
  const handleSignal = useCallback(async (sessionId: string, row: SignalRow) => {
    ...
    if (row.signal_type === "offer" && roleRef.current === "callee") {
      await peer.setRemoteDescription(new RTCSessionDescription(row.payload as RTCSessionDescriptionInit));
  ...
  const setupPeer = useCallback(async (session: CallSession, role: "caller" | "callee") => {
    ...
    setActive(session); activeRef.current = session; setStatus(role === "caller" ? "Calling…" : "Connecting…");
    ...
    const audio = document.createElement("audio"); audio.autoplay = true; audio.playsInline = true; document.body.appendChild(audio); remoteAudioRef.current = audio;
    peer.ontrack = (event) => { audio.srcObject = event.streams[0]; void audio.play().catch(() => undefined); setStatus("Connected"); };
    peer.onconnectionstatechange = () => { if (peer.connectionState === "connected") setStatus("Connected"); if (["failed","closed"].includes(peer.connectionState)) endLocal("Call ended"); };
    peer.onicecandidate = (event) => { if (event.candidate) void sendSignal(session.session_id, "ice", event.candidate.toJSON() as unknown as Record<string, unknown>); };

    const channel = supabase.channel(`call-signals-${session.session_id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "call_signals", filter: `session_id=eq.${session.session_id}` }, (payload) => void handleSignal(session.session_id, payload.new as SignalRow))
      .subscribe();
    ...
    for (const row of existing.data || []) await handleSignal(session.session_id, row as SignalRow);
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(({ data }) => {
      const user = data.user; if (!user) return; userIdRef.current = user.id;
      sessionChannel = supabase.channel(`incoming-calls-${user.id}`)
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "call_sessions", filter: `callee_user_id=eq.${user.id}` }, (payload) => {
          const row = payload.new as IncomingRow; if (row.status === "active" && !activeRef.current) setIncoming(row);
        }).subscribe();
    });
    const auth = supabase.auth.onAuthStateChange((_event, session) => { userIdRef.current = session?.user?.id || ""; });
    return () => { sessionChannel?.unsubscribe(); auth.data.subscription.unsubscribe(); };
  }, []);
  ...
  useEffect(() => {
    const intercept = (event: MouseEvent) => {
      ...
      event.preventDefault(); event.stopPropagation(); setChooserPhone(anchor.href.replace(/^tel:/i, "")); setNotice("");
    };
  ...
  async function startInApp() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("loadlink-call-service", { body: { action: "start", conversation_id: conversation } });
      if (error) throw error;
      const session = data?.session as CallSession | undefined; if (!session?.session_id) throw new Error("Call could not start.");
      setChooserPhone(""); await setupPeer(session, "caller");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Call could not start."); }
  ...
  async function acceptIncoming() {
    ...
    try {
      ...
      const session: CallSession = { session_id: incoming.id, max_seconds: incoming.max_seconds, remaining_seconds: Math.max(0, incoming.max_seconds - elapsed), premium: incoming.max_seconds > 1200, started_at: incoming.started_at };
      setIncoming(null); await setupPeer(session, "callee");
    } catch (error) { setNotice(error instanceof Error ? error.message : "Call could not connect."); }
    finally { setBusy(false); }
  }
  ...
  async function hangup(reason = "ended") {
    ...
    if (current) {
      ...
      await supabase.functions.invoke("loadlink-call-service", { body: { action: "end", session_id: current.session_id, reason } }).catch(() => undefined);
    }
    endLocal("Call ended");
  }
  ...
  async function declineIncoming() {
    if (!incoming) return;
    await supabase.functions.invoke("loadlink-call-service", { body: { action: "end", session_id: incoming.id, reason: "declined" } }).catch(() => undefined);
    setIncoming(null);
  }
