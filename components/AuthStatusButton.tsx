...
export default function AuthStatusButton({ darkMode, className = "" }: { darkMode: boolean; className?: string }) {
  ...
  if (account.user) {
    ...
    return (
      <Link href="/account/settings" aria-label={`Signed in as ${email}. Open profile settings`} title={`Signed in as ${email}`} className={`${base} border-[#f6b800]/55 bg-black text-[#f6b800] shadow-[0_8px_20px_rgba(0,0,0,.14)]`}>
        {account.profile.avatar_url ? <img src={account.profile.avatar_url} alt="" className="h-full w-full object-cover" /> : <LoadLinkGearIcon />}
      </Link>
    );
  }

  return (
    <Link href={loginHref(pathname || "/")} aria-label="Log in or sign up" title="Log in / Sign up" className={`${base} ${darkMode ? "border-yellow-400/60 bg-yellow-400 text-black" : "border-black/10 bg-white text-black shadow-[0_8px_18px_rgba(0,0,0,0.08)]"}`}>
