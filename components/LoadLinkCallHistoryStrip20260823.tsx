...
export default function LoadLinkCallHistoryStrip20260823(){
  ...
  useEffect(()=>{
    ...
    const load=async(reveal=false)=>{
      ...
      if(active){setRows(nextRows);setAnswers(answerTimes);setIdentities(Object.fromEntries(identityPairs));if(reveal&&nextRows.length){window.setTimeout(()=>{const viewport=document.querySelector<HTMLElement>(".loadlink-message-viewport");viewport?.scrollTo({top:viewport.scrollHeight,behavior:"smooth"});},120);}}
    };
    ...
    const refresh=()=>window.setTimeout(()=>void load(false),120);const reveal=()=>window.setTimeout(()=>void load(true),80);
    ...
    return()=>{active=false;window.clearInterval(timer);observer.disconnect();window.removeEventListener("popstate",refresh);window.removeEventListener("loadlink-call-history-updated",reveal as EventListener);window.removeEventListener("loadlink-account-state-changed",refresh as EventListener);};
  },[]);
