...
export default function DealerMarketing({ darkMode, context, inventory, insights }: { darkMode: boolean; context: DealerWorkspaceState; inventory: DealerInventoryItem[]; insights: DealerInsight[] }) {
  ...
  useEffect(() => {
    void loadStatuses(); void loadCalendarData();
    ...
    <Surface darkMode={darkMode} className="overflow-hidden rounded-3xl">
      <div className="p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><div className="text-[10px] font-black uppercase tracking-[.12em] opacity-35">Reach</div><h2 className="mt-1 text-[24px] font-black tracking-[-.04em]">Status & updates</h2><p className="mt-1 max-w-xl text-sm leading-6 opacity-50">New stock, offers and dealership updates for people following your showroom.</p></div>
      ...
      <div className="grid grid-cols-2 border-t border-current/10 sm:grid-cols-4"><Mini label="Status live" value={activeStatus} /><Mini label="Followers" value={followers} /><Mini label="Updates" value={publishedUpdates} /><Mini label="Campaigns" value={activeCampaigns} /></div>
    </Surface>
...
function Tab({ active, subtle, onClick, children }: { active: boolean; subtle?: boolean; onClick: () => void; children: string }) {
  return <button type="button" onClick={onClick} className={`h-9 rounded-full border px-4 text-xs font-black transition ${active ? "border-current bg-current text-black dark:text-white" : subtle ? "border-current/10 opacity-45 hover:opacity-80" : "border-current/10 opacity-60 hover:opacity-100"}`}>{children}</button>;
}
function Mini({ label, value }: { label: string; value: string | number }) { return <div className="border-b border-r border-current/10 p-4"><div className="text-[9px] font-black uppercase tracking-[.1em] opacity-35">{label}</div><div className="mt-1 text-xl font-black tracking-[-.03em]">{value}</div></div>; }
...
function StatusList({ darkMode, items, canWrite, canPublish, onChanged }: { darkMode: boolean; items: DealerStatus[]; canWrite: boolean; canPublish: boolean; onChanged: () => Promise<void> }) {
  ...
  async function action(kind: "repost" | "remove" | "publish", id: string) { setBusy(`${kind}-${id}`); setError(""); try { await dealerFetch("/api/dealer/statuses", { method: "POST", body: JSON.stringify({ action: kind, status_id: id }) }); await onChanged(); } catch (e) { setError(e instanceof Error ? e.message : "Status could not be changed."); } finally { setBusy(""); } }
  return <Surface darkMode={darkMode} className="overflow-hidden rounded-3xl"><div className="border-b border-current/10 px-4 py-4 sm:px-5"><SectionHeading title="Dealer Status" detail="24-hour dealership content. Photos, short videos, vehicle cards, text and promotions." />{error ? <p className="mt-2 text-sm font-bold text-red-500">{error}</p> : null}</div>{items.length ? <div className="divide-y divide-current/10">{items.map((item) => { const completion = item.views ? Math.round((item.completed_views / item.views) * 100) : 0; const ended = new Date(item.expires_at).getTime() <= Date.now() || item.publication_status === "expired" || item.publication_status === "removed"; return <div key={item.id} className="grid gap-3 px-4 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:px-5"><div className="flex min-w-0 items-center gap-3">{item.media_url ? <div className="h-14 w-12 shrink-0 overflow-hidden rounded-xl bg-black/10">{item.content_type === "video" ? <video src={item.media_url} muted className="h-full w-full object-cover" /> : <img src={item.media_url} alt="" className="h-full w-full object-cover" />}</div> : <div className="flex h-14 w-12 shrink-0 items-center justify-center rounded-xl border border-current/10 text-[9px] font-black uppercase">{item.content_type}</div>}<div className="min-w-0"><div className="truncate text-sm font-black">{item.title || item.listing_title || `${item.content_type} status`}</div><div className="mt-1 text-xs opacity-45">{item.publication_status.replaceAll("_", " ")} · {ended ? "ended" : `expires ${new Date(item.expires_at).toLocaleString("en-ZA", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`}</div>{canWrite ? <div className="mt-2 flex gap-3">{canPublish && item.moderation_status !== "approved" && item.publication_status !== "removed" ? <button disabled={Boolean(busy)} onClick={() => void action("publish", item.id)} className="text-[11px] font-black underline underline-offset-4 disabled:opacity-30">Publish</button> : null}<button disabled={Boolean(busy)} onClick={() => void action("repost", item.id)} className="text-[11px] font-black underline underline-offset-4 disabled:opacity-30">Repost</button>{item.publication_status !== "removed" ? <button disabled={Boolean(busy)} onClick={() => void action("remove", item.id)} className="text-[11px] font-black opacity-45 disabled:opacity-30">Remove</button> : null}</div> : null}</div></div><div className="grid grid-cols-4 gap-4 text-right text-xs"><Stat value={item.views} label="views" /><Stat value={`${completion}%`} label="complete" /><Stat value={item.vehicle_opens} label="opens" /><Stat value={item.messages_generated} label="leads" /></div></div>; })}</div> : <EmptyState title="No Dealer Status yet" detail="Post a new arrival, short video, vehicle card, text update or promotion when there is something worth showing." />}</Surface>;
}
