...
async function readVideoDuration(file: File) {
  return new Promise<number>((resolve, reject) => {
    ...
    video.onerror = () => { URL.revokeObjectURL(url); reject(new Error("This video could not be read.")); }; video.src = url;
  });
}
...
export default function DealerStatusComposer({ darkMode, open, onClose, inventory, context, onDone }: { darkMode: boolean; open: boolean; onClose: () => void; inventory: DealerInventoryItem[]; context: DealerWorkspaceState; onDone: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (type === "photo" || type === "video") {
        if (!file && !selectedMedia) throw new Error("Choose media for this Status.");
        ...
        if (file) {
          ...
          if (!allowed.includes(file.type as never)) throw new Error(type === "video" ? "Use an MP4, WebM or MOV video." : "Use a JPEG, PNG or WebP photo.");
          ...
          const uploaded = await uploadDealerFile({ bucket: "dealership-status-media", dealershipId: context.dealership_id, file, allowedTypes: allowed, maxBytes: 60 * 1024 * 1024, folder: "status" });
          storagePath = uploaded.storage_path; uploadedPath = storagePath; mime = uploaded.mime; filename = uploaded.filename;
        }
        if (type === "video" && selectedMedia && (!duration || duration > 60.5)) throw new Error("This saved video does not have a valid duration. Upload it again before using it in Status.");
      }
      ...
      uploadedPath = ""; setForm({ title: "", body: "", listing_id: "", cta_label: "View vehicle", starts_at: "" }); setFile(null); setSelectedMedia(null); onDone(); onClose();
  ...
  return <Modal open={open} onClose={onClose} darkMode={darkMode} title="Create Dealer Status">
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{DEALER_STATUS_TYPES.map((item) => <button type="button" key={item.value} onClick={() => { setType(item.value); setFile(null); setSelectedMedia(null); }} className={`border px-3 py-3 text-left ${type === item.value ? "border-[#f6b800]" : "border-current/10"}`}><div className="text-xs font-black">{item.label}</div><div className="mt-1 text-[10px] leading-4 opacity-50">{item.detail}</div></button>)}</div>
    <div className="mt-5 grid gap-3">
      {(type === "photo" || type === "video") ? <div><input ref={inputRef} type="file" className="hidden" accept={type === "video" ? "video/mp4,video/webm,video/quicktime" : "image/jpeg,image/png,image/webp"} onChange={(e) => { const next=e.target.files?.[0] || null; setFile(next); setSelectedMedia(null); setMediaNote(""); if(next && type==="photo") void readImageSize(next).then(({width,height})=>{ if(width<1080 || height<1080) setMediaNote("This photo is relatively small. A higher-resolution image will look cleaner in Status."); else if(width>height*2.2) setMediaNote("This photo is very wide. Check the crop before publishing."); }).catch(()=>{}); }} /><SecondaryButton darkMode={darkMode} type="button" onClick={() => inputRef.current?.click()}>{file ? file.name : type === "video" ? "Choose video" : "Choose photo"}</SecondaryButton></div> : null}
      {mediaNote ? <p className="text-xs font-semibold opacity-55">{mediaNote}</p> : null}
      {(type === "photo" || type === "video") && mediaItems.some((m) => m.media_type === (type === "video" ? "video" : "image")) ? <div><div className="mb-2 text-[11px] font-black uppercase tracking-[.08em] opacity-40">Recent media</div><div className="grid grid-cols-4 gap-2 sm:grid-cols-6">{mediaItems.filter((m) => m.media_type === (type === "video" ? "video" : "image")).slice(0, 12).map((item) => <button type="button" key={item.id} onClick={() => { setSelectedMedia(item); setFile(null); }} className={`aspect-square overflow-hidden border ${selectedMedia?.id === item.id ? "border-current" : "border-current/10"}`}>{item.media_type === "video" ? <video src={item.url} muted playsInline className="h-full w-full object-cover" /> : <img src={item.url} alt={item.label || "Dealer media"} className="h-full w-full object-cover" />}</button>)}</div></div> : null}
      ...
      <label className="text-xs font-black">Title<Input darkMode={darkMode} className="mt-1" maxLength={80} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder={type === "promotion" ? "Weekend price drop" : "Optional title"} /></label>
      <label className="text-xs font-black">Caption<Textarea darkMode={darkMode} className="mt-1" maxLength={400} value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} placeholder={type === "text" ? "Dealership update" : "Optional caption"} /></label>
      <div><SecondaryButton darkMode={darkMode} type="button" onClick={draftWording}>Draft wording</SecondaryButton><p className="mt-1 text-[11px] font-semibold opacity-45">Uses the selected Status type and vehicle only. Review it before publishing.</p></div>
      <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black">Call to action<Input darkMode={darkMode} className="mt-1" value={form.cta_label} onChange={(e) => setForm({ ...form, cta_label: e.target.value })} /></label><label className="text-xs font-black">Schedule<Input darkMode={darkMode} className="mt-1" type="datetime-local" value={form.starts_at} onChange={(e) => setForm({ ...form, starts_at: e.target.value })} /></label></div>
    </div>
    {error ? <div className="mt-4 text-sm font-bold text-red-500">{error}</div> : null}
    <div className="mt-5 flex justify-end"><PrimaryButton type="button" disabled={busy} onClick={submit}>{busy ? "Preparing Status…" : "Submit Status"}</PrimaryButton></div>
  </Modal>;
}
