...
function parseHours(raw?: string | null): HoursState {
  ...
  try {
    ...
    if (/mon.*fri/.test(compact)) return defaultHours;
    return defaultHours;
  }
}
...
export default function DealerShowroom({darkMode,profile,context,onProfile}:{darkMode:boolean;profile:DealerProfile;context:DealerWorkspaceState;onProfile:(profile:DealerProfile)=>void}){
  ...
  const missingRequired=useMemo(()=>{
    ...
    return missing;
  },[form,profile.profile_image_url,hoursValid]);
  ...
  const status=form.is_public?"Live on LoadLink":profileComplete&&canPublish?"Ready to go live":"Setup in progress";

  async function save(){
    if(!profileComplete){
      setDetailsOpen(true); setHoursOpen(true);
      setMessage(`Before LoadLink can save this Dealer profile as public, add: ${missingRequired.join(", ")}.`);
      return;
    }
    if(!canPublish){setMessage("Your Dealer package or business verification is not approved yet, so this showroom cannot go public.");return;}
    ...
    try{
      const data=await dealerFetch<{profile:DealerProfile}>("/api/dealer/showroom",{method:"POST",body:JSON.stringify({action:"save",...form,trading_hours:serialiseHours(hours),is_public:true})});
      onProfile(data.profile); setForm(toForm(data.profile)); setHours(parseHours(data.profile.trading_hours));
      setMessage("Saved. Your public showroom has been updated with these changes.");
    }catch(error){
      const detail=error instanceof Error?error.message:"";
      setMessage(detail&&detail!=="LoadLink could not complete this request."?detail:"Your Dealer profile could not be saved. Check the highlighted required details and try again.");
  ...
  return <div className="mx-auto grid max-w-[1040px] gap-4 pb-28">
    ...
    {!profileComplete?<Surface darkMode={darkMode} className="rounded-[22px] p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-black">Finish your Dealer profile</h2><p className="mt-2 text-sm font-semibold leading-6 opacity-55">Customers will see these details on your posts and showroom. Posting stays locked until they are complete.</p></div><span className="shrink-0 rounded-full border border-current/12 px-3 py-1.5 text-[10px] font-black">{missingRequired.length} missing</span></div><div className="mt-4 flex flex-wrap gap-2">{missingRequired.map(item=><span key={item} className="rounded-full border border-current/12 px-3 py-2 text-[10px] font-black capitalize">{item}</span>)}</div></Surface>:null}
