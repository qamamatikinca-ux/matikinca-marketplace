...
const validSections = new Set<DealerSection>([
  "overview", "inventory", "leads", "messages", "analytics", "customers", "marketing", "team", "showroom", "verification", "billing", "reviews", "activity", "settings", "support",
]);

function initialSection(value: string | null): DealerSection {
  return value && validSections.has(value as DealerSection) ? (value as DealerSection) : "overview";
}
...
export default function DealerWorkspace() {
  ...
  const [section, setSection] = useState<DealerSection>(() => initialSection(params.get("section")));
  ...
  const refreshCore = useCallback(async () => {
    ...
    try {
      const [summaryData, intelligence, leadData, inventoryData, appointmentData, teamData] = await Promise.allSettled([
        ...
        // Home only needs enough stock to calculate useful attention signals. The full
        // inventory view performs its own paginated request when the user opens Stock.
        dealerFetch<{ items: DealerInventoryItem[] }>("/api/dealer/inventory?page=1&page_size=12&stock=all&publication=all&moderation=all&sort=newest"),
  ...
  const boot = useCallback(async () => {
    ...
    try {
      let data = await dealerFetch<{ context: DealerWorkspaceState | null; profile: DealerProfile | null }>("/api/dealer/context");
      ...
      setError(e instanceof Error ? e.message : "Dealer could not be opened.");
    } finally {
      setLoading(false);
    }
  }, [refreshCore]);
  ...
  const validSection = useMemo(() => validSections.has(section) ? section : "overview", [section]);

  function navigate(next: DealerSection) {
    setSection(next);
    window.history.replaceState({}, "", `/dealer?section=${next}`);
  }

  function openSuccess(title: string, message: string) {
    setSuccessText({ title, message });
    setSuccess(true);
  ...
  function addVehicle() {
    if (!context?.dealership_id) return;
    window.location.assign(`/list-your-vehicle?entry=vehicle&dealership=${context.dealership_id}`);
  }

  if (loading) return <main className={`min-h-screen ${darkMode ? "bg-black text-white" : "bg-[#f4f0e7] text-black"}`}><div className="mx-auto flex min-h-screen max-w-lg items-center justify-center px-4"><Surface darkMode={darkMode} className="w-full p-8 text-center"><div className="mx-auto h-9 w-9 animate-spin rounded-full border-2 border-current/10 border-t-[#f6b800]" /><div className="mt-4 text-sm font-black">Loading LoadLink</div></Surface></div></main>;
  ...
  return <><SubmissionSuccess open={success} title={successText.title} message={successText.message} /><DealerShell darkMode={darkMode} toggleTheme={toggleTheme} profile={profile} context={context} section={validSection} setSection={navigate} onAddVehicle={addVehicle}>
    ...
    {validSection === "showroom" ? <DealerShowroom darkMode={darkMode} profile={profile} context={context} onProfile={(next) => { setProfile(next); openSuccess("Dealer page saved", "Your dealership details are up to date."); }} /> : null}
