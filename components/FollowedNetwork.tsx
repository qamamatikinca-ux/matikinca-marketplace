...
import { FollowedProfile, getFollowedProfiles, mergeFollowedProfiles } from "@/lib/following";
...
export default function FollowedNetwork({ darkMode }: { darkMode: boolean }) {
  ...
  useEffect(() => {
    let active = true;
    const sync = () => setProfiles(getFollowedProfiles());
    sync();
    window.addEventListener("loadlink-following-changed", sync);
    window.addEventListener("storage", sync);

    browserSupabase().auth.getSession().then(async ({ data }) => {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className={`mt-3 max-w-2xl text-sm leading-7 md:text-base ${muted}`}>Dealerships, contractors and opportunity posters you follow appear here, together with the updates you chose to receive.</p>
          </div>
          <Link href="/jobs" className="inline-flex h-11 items-center justify-center rounded-xl border border-[#f6b800] px-5 text-xs font-black uppercase tracking-[.12em] text-[#9b7100]">Discover people and businesses</Link>
        </div>
        ...
        {visible.length ? (
          ...
          <div className={`mt-7 grid gap-5 rounded-[26px] border p-5 md:grid-cols-[1fr_auto] md:items-center md:p-7 ${surface}`}>
            <div>
              ...
              <p className={`mt-2 max-w-2xl text-sm leading-6 ${muted}`}>Follow a dealership or opportunity poster and choose whether you want new listings, business updates or price-change alerts.</p>
            </div>
