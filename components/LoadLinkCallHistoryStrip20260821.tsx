...
export default function LoadLinkCallHistoryStrip20260821() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabase.from("call_sessions").select("id,caller_user_id,callee_user_id,started_at,ended_at,status,end_reason").eq("conversation_id", conversation).neq("status", "active").order("started_at", { ascending: false }).limit(8);
      if (error || !active) return;
      const rawRows = (data || []) as CallRow[];
      const sessionIds = rawRows.map((row) => row.id);
      const answerTimes: Record<string, string> = {};
      if (sessionIds.length) {
        const { data: answerData } = await supabase.from("call_signals").select("session_id,created_at").in("session_id", sessionIds).eq("signal_type", "answer").order("created_at", { ascending: true });
        ((answerData || []) as AnswerRow[]).forEach((answer) => { if (!answerTimes[answer.session_id]) answerTimes[answer.session_id] = answer.created_at; });
      }
      ...
      const identityPairs = await Promise.all(nextRows.map(async (row) => {
        ...
        return [row.id, { name: String((identity as { full_name?: string } | null)?.full_name || "LoadLink contact"), avatarUrl: String((identity as { avatar_url?: string | null } | null)?.avatar_url || "") }] as const;
      }));
      if (active) { setRows(nextRows); setAnswers(answerTimes); setIdentities(Object.fromEntries(identityPairs)); }
    };

    void load();
    timer = window.setInterval(() => void load(), 3500);
    const observer = new MutationObserver(findHost);
    observer.observe(document.body, { childList: true, subtree: true });
    const refresh = () => window.setTimeout(() => void load(), 90);
    window.addEventListener("popstate", refresh);
    window.addEventListener("loadlink-call-history-updated", refresh as EventListener);
    return () => { active = false; window.clearInterval(timer); observer.disconnect(); window.removeEventListener("popstate", refresh); window.removeEventListener("loadlink-call-history-updated", refresh as EventListener); };
  }, []);
  ...
  return createPortal(
    <div className="ll-final-call-history" aria-label="Call activity in this chat">
      {visibleRows.map((row) => {
        ...
        return <div className="ll-final-call-event" key={row.id} data-loadlink-call-event="true"><span className="ll-final-call-icon" aria-hidden="true"><LoadLinkIcon name="phone" size={14} strokeWidth={2}/></span><span className="ll-final-call-event-copy"><strong>{titleFor(row, outgoing, identity.name, answered)}</strong><small>{connected ? durationLabel(answers[row.id], row.ended_at) : "No audio connection"} · {outgoing ? "Outgoing" : "Incoming"}</small></span></div>;
      })}
    </div>, host,
  );
}
