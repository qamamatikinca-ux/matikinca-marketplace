      ...
      <div className="mx-auto max-w-6xl">
        {!compact ? (
          <div className="mb-7 max-w-3xl">
            <h2 className="text-4xl font-black tracking-[-0.045em] md:text-6xl">Choose your listing plan first</h2>
            <p className={`mt-4 max-w-2xl text-sm leading-7 md:text-base ${darkMode ? "text-white/60" : "text-black/60"}`}>Job posting remains free. Vehicle sellers choose Manual, Pro or Dealer before selecting a truck, so the correct photo limit, analytics and dealership tools are applied from the start.</p>
          </div>
        ) : null}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {plans.map((plan) => {
            ...
            return (
              <article key={plan.id} className={`relative overflow-hidden rounded-[28px] border p-5 md:p-6 ${selected ? "border-[#f6b800] ring-2 ring-[#f6b800]/25" : plan.highlight ? "border-[#f6b800] bg-black text-white shadow-[0_20px_60px_rgba(184,137,0,.18)]" : darkMode ? "border-white/10 bg-[#0d0d0d] text-white" : "border-black/10 bg-white text-black"}`}>
                ...
                <div className="mt-4 flex items-end gap-2"><span className="text-4xl font-black tracking-[-0.05em]">{plan.price}</span><span className={`pb-1 text-xs font-bold ${plan.highlight ? "text-white/50" : darkMode ? "text-white/45" : "text-black/45"}`}>{plan.billing}</span></div>
                ...
                <ul className="space-y-3">{plan.features.map((feature) => <li key={feature} className="flex items-start gap-3 text-sm font-semibold leading-5"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#f6b800] text-[11px] font-black text-black">✓</span><span>{feature}</span></li>)}</ul>
                {selectable ? (
                  ...
                  <Link href={plan.id === "dealer" ? "/dealership/loadlink-commercial-centurion" : `/list-your-truck?plan=${plan.id}`} className={`mt-6 flex h-12 items-center justify-center rounded-xl text-xs font-black uppercase tracking-[0.13em] ${plan.highlight ? "bg-[#f6b800] text-black" : "border border-[#f6b800] text-[#a87900]"}`}>{plan.id === "dealer" ? "View dealer example" : "Choose this option"}</Link>
                )}
              </article>
            );
          })}
