...
export default function ResetPasswordPage() {
  ...
  return (
    <AuthShell title={state === "invalid" ? "Request a new reset link" : "Reset your password"} description={state === "invalid" ? "That recovery link can no longer be used. Nothing on your account was changed." : "Choose a strong new password. After the reset, LoadLink returns you to sign in securely."} status="Password recovery">
      {state === "ready" ? (
        <form onSubmit={update} className="grid gap-4">
          <div className={`rounded-2xl border p-4 ${darkMode ? "border-emerald-500/20 bg-emerald-500/[.07]" : "border-emerald-600/20 bg-emerald-500/[.06]"}`}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-500/12 font-black text-emerald-500">✓</span><div><p className="text-sm font-black">Reset link verified</p><p className={`mt-1 text-xs font-semibold leading-5 ${muted}`}>You can safely create a new password for this account.</p></div></div></div>
          ...
          <button type="submit" disabled={busy} className="h-13 rounded-2xl bg-[#f6b800] px-5 text-sm font-black text-black shadow-[0_12px_30px_rgba(246,184,0,.17)] disabled:opacity-45">{busy ? "Updating securely…" : "Set new password"}</button>
        </form>
      ) : null}

      {state === "checking" ? <div className="py-8 text-center"><div className="mx-auto h-11 w-11 animate-spin rounded-full border-2 border-current/10 border-t-[#f6b800]" /><p className={`mt-4 text-sm font-semibold ${muted}`}>{message}</p><p className={`mx-auto mt-2 max-w-sm text-xs font-semibold leading-5 ${darkMode ? "text-white/34" : "text-black/34"}`}>This normally takes only a moment.</p></div> : null}

      {state === "invalid" ? <div><div className={`rounded-2xl border p-4 ${darkMode ? "border-amber-500/20 bg-amber-500/[.06]" : "border-amber-600/20 bg-amber-500/[.06]"}`}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-500/12 font-black text-amber-500">!</span><div><p className="text-sm font-black">Link unavailable</p><p className={`mt-1 text-xs font-semibold leading-5 ${muted}`}>{message}</p></div></div></div><Link href="/forgot-password" className="mt-5 flex h-13 w-full items-center justify-center rounded-2xl bg-[#f6b800] px-5 text-sm font-black text-black">Send a fresh reset link</Link><Link href="/login" className={`mt-3 flex h-12 w-full items-center justify-center rounded-2xl border px-5 text-sm font-black ${darkMode ? "border-white/12 text-white/72" : "border-black/12 text-black/72"}`}>Return to sign in</Link></div> : null}
