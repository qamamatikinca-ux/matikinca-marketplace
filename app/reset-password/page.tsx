    ...
    <AuthShell title="Choose a new password" description="Create a strong new password. After the reset, LoadLink returns you to sign in with the new password.">
      {state === "ready" ? (
        <form onSubmit={update} className="grid gap-4">
          <div className="grid gap-2"><label className="grid gap-2"><span className="text-sm font-bold">New password</span><input className={input} type="password" autoComplete="new-password" maxLength={128} value={password} onChange={(event) => setPassword(event.target.value)} required /></label><PasswordStrengthMeter password={password} darkMode={darkMode} /></div>
