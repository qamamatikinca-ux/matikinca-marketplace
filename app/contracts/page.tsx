      ...
      <div className="mx-auto w-full max-w-6xl px-4 pb-16 pt-7 sm:px-6 md:pt-10">
        ...
        <section className="mt-6 grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(240px,.55fr)]">
          <label className={`flex h-14 items-center rounded-[16px] border px-4 ${surface}`}>
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search contracts, vehicle type or company" className="w-full bg-transparent text-base font-semibold outline-none placeholder:opacity-40" />
          </label>
          <SouthAfricaLocationInput id="loadlink-contract-location" value={city} onChange={setCity} darkMode={darkMode} placeholder="City, town or province" ariaLabel="Contract location" className={`h-14 w-full rounded-[16px] border px-4 text-base font-semibold outline-none ${surface}`} />
        </section>
