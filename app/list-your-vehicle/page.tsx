...
export default function ListYourVehiclePage() {
  ...
  if (!entryMode) {
    return (
      <main className={`min-h-screen ${page}`} data-loadlink-vehicle-portal="source-v8-restored">
        ...
        <section className="relative flex min-h-[600px] w-full items-end overflow-hidden bg-black text-white md:min-h-[620px]">
          ...
          <div className="relative z-10 mx-auto w-full max-w-6xl px-5 pb-10 pt-20 text-center">
            <h1 className="mx-auto max-w-4xl text-[clamp(3rem,12vw,5.7rem)] font-black leading-[.94] tracking-[-.055em]">
              List your truck, trailer or mobile unit.
            </h1>
            <p className="mx-auto mt-5 max-w-2xl text-sm font-semibold leading-7 text-white/72 md:text-base">
              Create a LoadLink listing in a guided flow, or browse commercial stock already on the marketplace.
            </p>
            <div className="mx-auto mt-7 grid w-full max-w-[620px] gap-2.5 sm:grid-cols-3">
              <button
                type="button"
                onClick={() => openEntry("vehicle")}
                className="min-h-[54px] rounded-full bg-[#f6b800] px-5 text-xs font-black uppercase tracking-[.08em] text-black"
              >
                List vehicle
              </button>
              <button
                type="button"
                onClick={() => openEntry("mobile-unit")}
                className="min-h-[54px] rounded-full border border-white/30 bg-black/58 px-5 text-xs font-black uppercase tracking-[.08em] text-white"
              >
                List mobile unit
              </button>
              <a
                href="#vehicle-marketplace"
                className="flex min-h-[54px] items-center justify-center rounded-full border border-white/30 bg-black/58 px-5 text-xs font-black uppercase tracking-[.08em] text-white"
              >
                Browse marketplace
              </a>
            </div>
      ...
      <main className={`min-h-screen ${page}`}>
        ...
        <section className="mx-auto w-full max-w-3xl px-4 py-10">
          <div className={`rounded-[24px] border p-6 ${darkMode ? "border-white/10 bg-[#080808]" : "border-black/10 bg-white"}`}>
            <p className="text-[10px] font-black uppercase tracking-[.16em] opacity-45">Posting status</p>
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <Link href="/list-your-vehicle#vehicle-marketplace" className="flex min-h-[54px] items-center justify-center rounded-full bg-[#f6b800] px-5 text-xs font-black text-black">
                Browse marketplace
              </Link>
              <Link href="/dealer" className="flex min-h-[54px] items-center justify-center rounded-full border border-current/15 px-5 text-xs font-black">
                Open Dealer
              </Link>
            </div>
          </div>
  ...
  return (
    <main className={page} data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored">
      <LoadLinkSiteHeader darkMode={darkMode} onToggleTheme={toggleTheme} />
      <LoadLinkVehicleWizardController20260825 darkMode={darkMode} />
      <div id="listing-form" className="scroll-mt-36">
        <LegacyVehicleListingPage />
      </div>
      <style jsx global>{`
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #listing-form > main > header,
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #listing-form > main > section:first-of-type,
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #listing-form #plans {
          display: none !important;
        }
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #vehicle-listing-form {
          padding-bottom: max(118px, calc(env(safe-area-inset-bottom) + 94px)) !important;
        }
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #vehicle-listing-form > section[hidden] {
          display: none !important;
        }
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] #vehicle-listing-form > section[data-loadlink-wizard-active="true"] {
          scroll-margin-top: 178px;
        }
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] input,
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] select,
        [data-loadlink-vehicle-listing-shell="source-step-flow-v8-restored"] textarea {
