...
function rankTitle(title: string, brand: string, model: string, year: string, width = 0, height = 0) {
  ...
  brandTokens.forEach((token) => {
    if (cleanTitle.includes(token)) score += 5;
  });
  modelTokens.forEach((token) => {
    if (cleanTitle.includes(token)) score += 9;
  });
  if (year && cleanTitle.includes(year)) score += 6;
...
async function searchCommons(query: string, brand: string, model: string, year: string) {
  ...
  const response = await fetch(endpoint, {
    headers: {
      "User-Agent": "LoadLink commercial vehicle reference image finder/2.0",
  ...
  return pages
    .map((page): Candidate | null => {
      ...
      return {
        score: rankTitle(page.title || "", brand, model, year, info.width || 0, info.height || 0),
...
export async function GET(request: Request) {
  ...
  const queries = [
    `${brand} ${model} ${year} truck`.trim(),
    `${brand} ${model} truck`.trim(),
    `${brand} ${model} tractor unit`.trim(),
  ];
  ...
  try {
    ...
    searches.forEach((result) => {
      ...
      result.value.forEach((candidate) => {
        const existing = unique.get(candidate.originalUrl);
        if (!existing || candidate.score > existing.score) unique.set(candidate.originalUrl, candidate);
      });
    });
    ...
    const matchConfidence = best.score >= 25 ? "high" : best.score >= 15 ? "medium" : "reference";
    return NextResponse.json({
      imageUrl: best.url,
      originalUrl: best.originalUrl,
      title: best.title,
      exactMatch: matchConfidence === "high",
      matchConfidence,
      credit: best.credit,
