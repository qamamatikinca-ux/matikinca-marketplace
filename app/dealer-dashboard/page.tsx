    ...
    <main className="min-h-screen bg-[#f4efe3] text-black">
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 md:px-8">
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[['Demo stock','9'],['Test inbox','Ready'],['Profile followers','Demo'],['Public showroom','Live']].map(([label,value]) => <div key={label} className="rounded-2xl border border-black/10 bg-white p-5"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-black/40">{label}</p><strong className="mt-2 block text-3xl font-black">{value}</strong></div>)}
        </div>
        <div className="mt-6 grid gap-5 lg:grid-cols-3">
          {[['Inventory management','Add, edit, archive and monitor every dealership vehicle.'],['Lead inbox','Keep enquiries together and assign them to sales staff.'],['Dealer promotions','Publish updates and promote selected vehicles to followers.'],['Team access','Give sales staff controlled access without sharing the owner login.'],['Performance analytics','Track showroom views, vehicle interest and response performance.'],['Public showroom','Control branding, contact details, business hours and stock order.']].map(([title,copy]) => <article key={title} className="rounded-[24px] border border-black/10 bg-white p-6"><h2 className="text-xl font-black">{title}</h2><p className="mt-2 text-sm leading-6 text-black/55">{copy}</p><Link href="/dealer" className="mt-5 inline-flex rounded-xl border border-[#f6b800] px-4 py-2 text-xs font-black uppercase text-[#8a6500]">Open functional centre</Link></article>)}
        </div>
      </section>
    </main>
  );
