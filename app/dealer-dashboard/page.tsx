...
export default function DealerDashboardPage() {
  ...
  const dealershipName = String(profile?.display_name || "LoadLink Commercial Centurion");
  return (
    <main className={darkMode ? "min-h-screen bg-black text-white" : "min-h-screen bg-[#f4efe3] text-black"}>
      <header className={`border-b ${darkMode ? "border-white/10 bg-black" : "border-black/10 bg-white"}`}><div className="grid h-20 grid-cols-[100px_1fr_100px] items-center px-4"><div className="flex items-center gap-2"><SiteMenu darkMode={darkMode}/><AuthStatusButton darkMode={darkMode}/></div><HomeLogoLink theme={darkMode ? "dark" : "light"}/><Link href="/dealership/loadlink-commercial-centurion" className="justify-self-end text-xs font-black uppercase">Public profile</Link></div></header>
      <section className="mx-auto max-w-7xl px-5 py-10 md:px-8">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#9d7300]">Dealer control panel</p><h1 className="mt-2 text-5xl font-black tracking-[-0.055em]">{dealershipName}</h1><p className={`mt-3 text-sm ${darkMode ? "text-white/55" : "text-black/55"}`}>Manage stock, leads, followers, promotions and public dealership visibility.</p>
        <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[['Active stock','9'],['Lead inbox','Ready'],['Profile followers','1 825'],['Public showroom','Live']].map(([label,value]) => <div key={label} className={`rounded-2xl border p-5 ${darkMode ? "border-white/10 bg-[#0d0d0d]" : "border-black/10 bg-white"}`}><p className={`text-[10px] font-black uppercase tracking-[0.14em] ${darkMode ? "text-white/40" : "text-black/40"}`}>{label}</p><strong className="mt-2 block text-3xl font-black">{value}</strong></div>)}
        </div>
        <div className="mt-6 grid gap-5 lg:grid-cols-3">
          {[['Inventory management','Add, edit, archive and monitor every dealership vehicle.'],['Lead inbox','Keep enquiries together and assign them to sales staff.'],['Dealer promotions','Publish updates and promote selected vehicles to followers.'],['Team access','Give sales staff controlled access without sharing the owner login.'],['Performance analytics','Track showroom views, vehicle interest and response performance.'],['Public showroom','Control branding, contact details, business hours and stock order.']].map(([title,copy]) => <article key={title} className={`rounded-[24px] border p-6 ${darkMode ? "border-white/10 bg-[#0d0d0d]" : "border-black/10 bg-white"}`}><h2 className="text-xl font-black">{title}</h2><p className={`mt-2 text-sm leading-6 ${darkMode ? "text-white/55" : "text-black/55"}`}>{copy}</p><Link href="/dealer" className="mt-5 inline-flex rounded-xl border border-[#f6b800] px-4 py-2 text-xs font-black uppercase text-[#8a6500]">Open functional centre</Link></article>)}
        </div>
      </section>
    </main>
  );
