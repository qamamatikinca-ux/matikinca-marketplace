...
begin
  ...
  join lateral (
    select ds.id, ds.title from public.dealership_statuses ds
    ...
    order by coalesce(ds.starts_at,ds.created_at) desc limit 1
  ) s on true
  where t.id=p_thread_id and (
    t.buyer_user_id=v_uid or j.user_id=v_uid or d.owner_user_id=v_uid or exists (
      select 1 from public.dealership_staff staff
      where staff.dealership_id=d.id and staff.user_id=v_uid and coalesce(staff.is_active,true)
    )
  ) limit 1;
end;
$$;
revoke all on function public.loadlink_chat_dealer_status(uuid) from public;
grant execute on function public.loadlink_chat_dealer_status(uuid) to authenticated;
