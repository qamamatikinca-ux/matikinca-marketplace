...
export default function JobsPortalPage() {
  ...
  const portalCopy = portalFilter === "contract"
    ...
    : portalFilter === "asset"
      ...
      : { eyebrow: "Find jobs portal", title: "Find paid logistics work", description: "For truck owners, drivers, mobile kitchens, mobile fridges, farming vehicles and mining transport operators looking for real posted jobs.", searchButton: "Search jobs", listLabel: "List a job", listHref: "/jobs/list", results: "Available jobs" };
    ...
    <main className={`min-h-screen scroll-smooth transition-colors duration-500 ${darkMode ? "bg-black text-white" : "bg-[#fff7df] text-black"}`}>
      ...
      <section className="relative min-h-[82vh] overflow-hidden border-b border-[#f6b800]/40">
        ...
        <div className="relative mx-auto flex min-h-[82vh] max-w-5xl flex-col justify-end px-5 pb-8 pt-20">
          <div className="max-w-4xl text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.95)]">
            <p className="mb-4 inline-flex border border-[#f6b800] bg-black/35 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[#f6b800]">{portalCopy.eyebrow}</p>
          ...
          <div className="mt-4">
            <Link href={portalCopy.listHref} className="inline-flex items-center gap-2 border border-[#f6b800] bg-[#f6b800] px-5 py-3 text-xs font-black uppercase tracking-wide text-black">
      ...
      <section id="matching-jobs" className="mx-auto max-w-5xl px-5 pb-12 pt-4">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.26em] text-[#b88900]">Available results</p>
            <h2 className="mt-2 text-4xl font-black tracking-[-0.05em]">{hasSearched ? `Matching ${portalCopy.results.toLowerCase()}` : portalCopy.results}</h2>
          </div>

          <Link href={portalCopy.listHref} className={`hidden border px-5 py-3 text-xs font-black uppercase tracking-wide md:inline-flex ${darkMode ? "border-[#f6b800] text-[#f6b800]" : "border-black text-black"}`}>{portalCopy.listLabel}</Link>
        </div>
    ...
    <section className={`border-b px-5 py-7 md:px-12 ${darkMode ? "border-white/10 bg-black text-white" : "border-black/10 bg-[#fff7df] text-black"}`}>
      <div className="mx-auto max-w-7xl">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[.22em] text-[#b88900]">Jobs in this portal</p>
            <h2 className="mt-2 text-3xl font-black">Featured and recent listings</h2>
          </div>
        </div>
  ...
  return (
    <div className={`mt-5 overflow-hidden border ${darkMode ? "border-white/10 bg-black" : "border-black/10 bg-[#f8f9ff]"}`}>
      ...
      <div className="grid grid-cols-3 border-t border-black/10">
        ...
        <Link href={`/messages?listing=${encodeURIComponent(job.id)}`} className="flex min-h-16 flex-col items-center justify-center gap-1.5 border-r border-black/10 bg-[#168eea] px-2 text-center text-xs font-black uppercase tracking-wide text-white"><MessageIcon /> Message</Link>
    ...
    <header className={`sticky top-0 z-50 border-b transition-colors duration-500 ${darkMode ? "border-white/10 bg-black" : "border-black/10 bg-white"}`}>
      <div className="grid h-20 w-full grid-cols-[92px_1fr_52px] items-center px-4">
        <div className="flex items-center gap-2">
          ...
          <Link href="/login" aria-label="Log in or sign up" className={`flex h-10 w-10 items-center justify-center rounded-full border ${darkMode ? "border-yellow-400/60 bg-yellow-400 text-black" : "border-black/10 bg-white text-black shadow-[0_8px_18px_rgba(0,0,0,0.08)]"}`}>
            <HeaderUserPlusIcon />
          </Link>
        </div>
